...
/**
function passwordHash($email, $password, $birthdate, $username, $timestamp) {
    $input = $email . $password . $birthdate . $username . $timestamp;
    $input = str_shuffle($input);
    return hash('sha256', $input);
}

function passwordVerify($email, $password, $birthdate, $username, $timestamp, $hash) {
    $input = $email . $password . $birthdate . $username . $timestamp;
    $input = str_shuffle($input);
    return hash_equals(hash('sha256', $input), $hash);
}
function print_p($p = '')
{
    echo '<pre>';
...
function textInText($p, $p2) {
    if(is_array($p)) {
        foreach ($p as $text) {
            if (strpos($p2, $text) !== false) {
                return true;
            }
        }
        return false;
    } else {
        return strpos($p2, $p) !== false;
    }
} 
 */
