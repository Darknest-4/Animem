...
function getBanner($arr)
{
  ...
  {
    if (file_exists(FOLDER_ASSETS . "uploads/banner/partner/" . $arr[$key]["image"]))
      $arr[$key]["image"] = ASSETSURL . "uploads/banner/partner/" . $arr[$key]["image"];
    if (file_exists(FOLDER_ASSETS . "uploads/banner/high/" . $arr[$key]["image"]))
      $arr[$key]["image"] = ASSETSURL . "uploads/banner/high/" . $arr[$key]["image"];
  }
  return $arr;
}
function viewBuilder($view, $data)
{
  extract($data);
  ...
  { 
    if (!is_array($value))
    {
      if (file_exists(FOLDER_TEMPLATE . getTheme() . DS . "Other" . DS . $value . ".phtml"))
        require_once(FOLDER_TEMPLATE . getTheme() . DS . "Other" . DS . $value . ".phtml");
      elseif (file_exists(FOLDER_TEMPLATE . getTheme() . DS . "SideBar" . DS . $value . ".phtml"))
        require_once(FOLDER_TEMPLATE . getTheme() . DS . "SideBar" . DS . $value . ".phtml");
      elseif (file_exists(FOLDER_TEMPLATE . getTheme() . DS . "Content" . DS . $key . DS . $value . ".phtml"))
        require_once(FOLDER_TEMPLATE . getTheme() . DS . "Content" . DS . $key . DS . $value . ".phtml");
    }
...
function viewBuilderWidget($Widget)
{
  foreach ($Widget as $SideBarKey => $SideBarValue)
  {
    if (in_array($SideBarValue["Widget"] . ".phtml", findfilefromdir(FOLDER_TEMPLATE . getTheme() . DS . "Widget")))
      if (file_exists(FOLDER_TEMPLATE . getTheme() . DS . "Widget" . DS . $SideBarValue["Widget"] . ".phtml"))
      {
        require_once(FOLDER_TEMPLATE . getTheme() . DS . "Widget" . DS . $SideBarValue["Widget"] . ".phtml");
        if (!empty($SideBarValue["Widget"]) && function_exists($SideBarValue["Widget"])) call_user_func($SideBarValue["Widget"], $SideBarValue);
      }
  }
}

function uploadersTableRepair()
{
  $upl = Select("SELECT `id`, `save` FROM `datasheet`");
  foreach ($upl as $key => $value)
  {
    $save = json_decode($value["save"], true);
    unset($save["_type"]);
    if (is_array($save["fansub"]) && !empty($save["fansub"]))
    {
      foreach ($save["fansub"] as $fk => $fansub)
      {
        if (!is_array($fansub))
          $fansubs[] = '' . $fansub . '';
        if (is_array($fansub))
          foreach ($fansub as $fansub)
    ...
    if (is_array($save["links"]) && !empty($save["links"]))
    {
      foreach ($save["links"] as $fl => $link)
      {
        if (!is_array($link))
          $links[] = '' . $link . '';
        if (is_array($link))
      ...
      var_export($upl[$key]["save"]);
      $upl[$key]["save"] = json_encode($save);
      unset($save);
      echo "<br />";
      var_export($upl[$key]["save"]);
      echo "<hr />";
      // Update("UPDATE `datasheet` SET `save`='{$upl[$key]["save"]}' WHERE `id` = {$upl[$key]["id"]};");
  }
}
...
function setVisitorsFromDatabase()
{
  ...
  if (!empty($ip))
  {
    ...
    $lastTime = time() - (30 * 60);
    $session_check = Select("SELECT `session` FROM `statistic__visits_today` WHERE `session`='{$_SESSION['session']}' && '{$firstTime}' <= `create_time` && '{$lastTime}' <= `last_time` LIMIT 1;");
